...
export default function GroupSelector({
  ...
  useEffect(() => {
    if (groups?.length && props.defaultValue?.length) {
      setState((val) => {
        return val.map((item) => ({ ...item, selected: props.defaultValue?.includes(item.id) }));
      });
    }
  }, [groups, props.defaultValue]);
  ...
  return (
    <div className={`mx-auto p-4 bg-gray-100 rounded-md max-h-[30rem] overflow-y-auto ${width}`}>
      {accountId && (
      ...
      <div className="p-2">
        ...
        {loading ? (
          <div className="items-center justify-center flex fle">
            <LoadingIndicator />
          </div>
        ) : state.length ? (
          <>
