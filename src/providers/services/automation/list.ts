...
export async function getListAutomationHistory(automationId: string): Promise<IListAutomationMessage[]> {
  return axios
    .get<IListAutomationMessage[]>(`/automation/list/${automationId}/history`)
    .then((response) => {
      return response.data;
    })
    .catch(handleError);
...
export async function editAutomationId(
  automationId: string,
  data: ICreateAutomationList
): Promise<ICreateAutomationList> {
  return axios
